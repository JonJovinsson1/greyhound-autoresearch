...
# A slightly broader sprint best-time pass helps a second band of close calls,
# but only when both dogs are experienced enough for the published marks to be
# trustworthy.
BEST_TIME_RERANK_BROAD_GAP_THRESHOLD = 0.01
BEST_TIME_RERANK_BROAD_MAX_DISTANCE = 350
...
def rerank_best_time_broad_close_calls(race, feature_rows, probs):
    if len(probs) < 2:
        return probs
    if not race["distance"] or race["distance"] > BEST_TIME_RERANK_BROAD_MAX_DISTANCE:
        return probs
    ...
    first_idx, second_idx = order[:2]
    gap = scores[first_idx] - scores[second_idx]
    if gap > BEST_TIME_RERANK_BROAD_GAP_THRESHOLD:
        return scores

    first = feature_rows[first_idx]
    second = feature_rows[second_idx]
    if min(first["career_starts"], second["career_starts"]) < BEST_TIME_RERANK_BROAD_MIN_STARTS:
        return scores
    if not first["has_best_time"] or not second["has_best_time"]:
        return scores

    if (first["best_time"] - second["best_time"]) >= BEST_TIME_RERANK_BROAD_EDGE:
        scores[first_idx], scores[second_idx] = scores[second_idx], scores[first_idx]
    return scores
